...
metadata: {"skill-runner":{"emoji":"🌐","requires":{"bins":["curl","jq"]}}}
---
...
**API Key** is stored in skill-runner config at `skills.entries.browser-cash.apiKey`.

If not configured, prompt the user:
> Get your API key from https://dash.browser.cash and run:
> ```bash
> skill-runner config set skills.entries.browser-cash.apiKey "your_key_here"
> ```

**Reading the key:**
```bash
BROWSER_CASH_KEY=$(skill-runner config get skills.entries.browser-cash.apiKey)
```

**Before first use**, check and install Playwright if needed:
```bash
if [ ! -d ~/skills/node_modules/playwright ]; then
  cd ~/skills && npm install playwright puppeteer-core
fi
```
...
## Using Browser.cash with Skill Runner
...
```bash
[ -d ~/skills/node_modules/playwright ] || (cd ~/skills && npm install playwright puppeteer-core)
```
...
### Note on Skill Runner browser tool

Skill Runner's native `browser` tool expects HTTP control server URLs, not raw WebSocket CDP. The `gateway config.patch` approach works when Skill Runner's browser control server proxies the connection. For direct Browser.cash CDP, use the exec approach above.
...
- Skill Runner runs scripts from `~/skills/` - install npm dependencies there
