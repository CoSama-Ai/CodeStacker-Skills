...
metadata: {"skill-runner":{"emoji":"🎤","os":["linux","darwin"]}}
---
...
Set `GEMINI_API_KEY` in environment (e.g., `~/.env` or `~/.skill-runner/.env`)
...
# With Skill Runner media
python ~/.claude/skills/gemini-stt/transcribe.py ~/.skill-runner/media/inbound/voice-message.ogg
```
...
For Skill Runner voice message handling:
