...
```bash
# Add to crontab
*/5 * * * * cd ~/skills && python3 skills/ham-radio-dx/dx-monitor.py watch --new-only --callsign YOUR_CALL >> /tmp/dx-alerts.log
```
...
```bash
# Add to crontab
0 9 * * * cd ~/skills && python3 skills/ham-radio-dx/dx-monitor.py digest >> ~/dx-digest-$(date +\%Y-\%m-\%d).txt
```

### Telegram Notifications

Integrate with Skill Runner message tool:

```bash
# When rare DX appears, send Telegram alert
python3 dx-monitor.py watch --new-only | grep -E "(VP8|ZL|VK|ZS|P5)" && \
  echo "🚨 Rare DX spotted!" | # Send via Skill Runner message tool
```

## Example Prompts for Skill Runner
