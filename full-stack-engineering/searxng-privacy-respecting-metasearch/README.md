# SearXNG Search Skill for Skill Runner
...
Configure the `SEARXNG_URL` environment variable in your Skill Runner config:
...
```bash
# Basic search
uv run ~/skills/skills/searxng/scripts/searxng.py search "query"

# More results
uv run ~/skills/skills/searxng/scripts/searxng.py search "query" -n 20

# Category search
uv run ~/skills/skills/searxng/scripts/searxng.py search "query" --category images

# JSON output (for scripts)
uv run ~/skills/skills/searxng/scripts/searxng.py search "query" --format json

# Time-filtered news
uv run ~/skills/skills/searxng/scripts/searxng.py search "latest AI news" --category news --time-range day
```
...
### Web Search
```bash
uv run ~/skills/skills/searxng/scripts/searxng.py search "rust programming language"
```

### Image Search
```bash
uv run ~/skills/skills/searxng/scripts/searxng.py search "sunset photography" --category images -n 10
```

### Recent News
```bash
uv run ~/skills/skills/searxng/scripts/searxng.py search "tech news" --category news --time-range day
```

### JSON Output for Scripts
```bash
uv run ~/skills/skills/searxng/scripts/searxng.py search "python tips" --format json | jq '.results[0]'
```
...
This skill is part of the Skill Runner ecosystem and follows the same license terms.
