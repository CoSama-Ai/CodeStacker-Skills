...
Use the local deepwork app (SQLite-backed) at `~/skills/deepwork/deepwork.js`.

## Bootstrap (if the script is missing)

If `~/skills/deepwork/deepwork.js` does not exist, bootstrap it from the public repo:
...
# Clone if missing
[ -d ~/skills/deepwork-tracker/.git ] || git clone https://github.com/adunne09/deepwork-tracker.git ~/skills/deepwork-tracker

# Ensure expected runtime path exists
mkdir -p ~/skills/deepwork
cp -f ~/skills/deepwork-tracker/app/deepwork.js ~/skills/deepwork/deepwork.js
chmod +x ~/skills/deepwork/deepwork.js
```
...
- Start a session (also starts a macOS Clock timer; default target 60m):
  - `~/skills/deepwork/deepwork.js start --target-min 60`
- Stop a session:
  - `~/skills/deepwork/deepwork.js stop`
- Check status:
  - `~/skills/deepwork/deepwork.js status`
- Generate a report:
  - Last 7 days (default): `~/skills/deepwork/deepwork.js report --days 7 --format text`
  - Telegram-ready last 7 days: `~/skills/deepwork/deepwork.js report --days 7 --format telegram`
  - Heatmap (optional): `~/skills/deepwork/deepwork.js report --mode heatmap --weeks 52 --format telegram`

## Chat workflows

### Start deep work
1) Run `~/skills/deepwork/deepwork.js start --target-min 60` (or another target if the user specifies it).
...
### Stop deep work
1) Run `~/skills/deepwork/deepwork.js stop`.
2) Reply with duration.

### Show deep work graph
1) Run `~/skills/deepwork/deepwork.js report --days 7 --format telegram`.
